...
@dataclass
class ProbeFindings:
    item_counts_by_scale:dict=field(default_factory=dict); candidate_item_limit:int|None=None; duplicate_ratio_by_scale:dict=field(default_factory=dict); errors:list=field(default_factory=list); latencies_ms:list[float]=field(default_factory=list)
...
def run_probe(transport:Transport,plan,profile:ProbeRequestProfile,probe_dir:Path,clock=None):
    ...
    for request in plan:
        started=timer(); response=transport.send(request); latencies.append((timer()-started)*1000); results.append((request,response))
