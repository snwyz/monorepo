...
REQUIRED_KEYS = (
    ...
    "detail_lat_param", "response_item_limit", "coordinate_system", "qps_limit", "seed_scale",
...
def load_field_mapping(path: Path) -> FieldMapping:
    ...
    try:
        ...
        if mapping.qps_limit <= 0 or mapping.response_item_limit <= 0 or not 0 < mapping.dense_ratio <= 1 or not 0 <= mapping.empty_sample_ratio <= 1 or not 0 < mapping.edge_margin_ratio < .5 or mapping.edge_expansion_max_hops < 0: raise ValueError
